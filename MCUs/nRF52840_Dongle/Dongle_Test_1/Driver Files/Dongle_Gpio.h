#ifndef DONGLE_BOARD_H
#define DONGLE_BOARD_H
 ...
 */
static __inline nrf_gpio_pin_dir_t nrf_gpio_pin_dir_get(uint32_t pin_number);
 ...
 */
static __inline nrf_gpio_pin_input_t nrf_gpio_pin_input_get(uint32_t pin_number);
 ...
 */
static __inline nrf_gpio_pin_pull_t nrf_gpio_pin_pull_get(uint32_t pin_number);
 ...
 */
static __inline void nrf_gpio_latches_read(uint32_t   start_port,
                                           uint32_t   length,
                                           uint32_t * p_masks);
 ...
 * @param p_masks    Pointer to output array where latch states will be stored.
 */
static __inline void nrf_gpio_latches_read_and_clear(uint32_t   start_port,
                                                     uint32_t   length,
                                                     uint32_t * p_masks);
 ...
 */
static __inline uint32_t nrf_gpio_pin_latch_get(uint32_t pin_number);
 ...
 * @param pin_number Pin number.
 */
static __inline void nrf_gpio_pin_latch_clear(uint32_t pin_number);
 ...
 */
static __inline bool nrf_gpio_pin_present_check(uint32_t pin_number);
...
{
    ASSERT(nrf_gpio_pin_present_check(*p_pin));
#if (GPIO_COUNT == 1)
    return NRF_P0;
#else
    if (*p_pin < P0_PIN_NUM)
